...
// function getMaxDayOfMonth(mm,yy){
// 	var ly1, ly2, tet1, jd1, jd2, mm1, yy1, result, i;
// 	if (mm < 12) {
// 		mm1 = mm + 1;
// 		yy1 = yy;
...
// 	if (tet1 <= jd1) { /* tet(yy) = tet1 < jd1 < jd2 <= 1.1.(yy+1) < tet(yy+1) */
// 		for (i = jd1; i < jd2; i++) {
// 			// console.log(findLunarDate(i + 1 , ly1));
// 			if (findLunarDate(i + 1 , ly1).day == 1) {
// 				return findLunarDate(i , ly1).day;
// 			}
// 		}
// 	} else if (jd1 < tet1 && jd2 < tet1) {  tet(yy-1) < jd1 < jd2 < tet1 = tet(yy) 
// 		ly1 = getYearInfo(yy - 1);
// 		for (i = jd1; i < jd2; i++) {
// 			// console.log(findLunarDate(i , ly1));
// 			if (findLunarDate(i + 1 , ly1).day == 1) {
// 				return findLunarDate(i , ly1).day;
// 			}
// 		}
...
// 		ly2 = getYearInfo(yy - 1);
// 		for (i = jd1; i < tet1; i++) {
// 			if (findLunarDate(i + 1 , ly2).day == 1) {
// 				return findLunarDate(i , ly2).day;
// 			}			
// 		}
// 		for (i = tet1; i < jd2; i++) {
// 			if (findLunarDate(i + 1 , ly1).day == 1) {
...
function getMaxDayOfMonth(dd,mm,yy){
	let date = new Date(yy, mm , dd);

	let lunarDate = getLunarDate(dd,mm + 1,yy);
	let totalDay = lunarDate.day;
	...
	for (; ; ) {
		...
		lunarDate = getLunarDate(d,m,y);
		
		if (lunarDate.day == 1) {
			return totalDay;
		}
		else{
			totalDay = lunarDate.day;
		}
	}
...
function getLunarMonth11(yy, timeZone) {
	...
	sunLong = getSunLongitudes(nm, timeZone); // sun longitude at local midnight
	if (sunLong >= 9) {
...
function getLeapMonthOffset(a11, timeZone) {
	...
	arc = getSunLongitudes(getNewMoonDay(k+i, timeZone), timeZone);
	do {
		last = arc;
		i++;
		arc = getSunLongitudes(getNewMoonDay(k+i, timeZone), timeZone);
 ...
 * After that, return 1, 2, 3 ... 
 */
function getSunLongitudes(dayNumber, timeZone) {
	return INT(SunLongitude_aa98(dayNumber - 0.5 - timeZone/24)/PI*6);
}
 ...
 * Algorithm from: "Astronomical Algorithms" by Jean Meeus, 1998
 */
function SunLongitude_aa98(jdn) {
...
function is_LeapMonth(lunarDate,lunarMonth,lunarYear,timeZone){
	var f_curent_date = convertLunar2Solar(lunarDate,lunarMonth,lunarYear,0,timeZone);
	// console.log(f_curent_date);
	var l_curent_date = convertLunar2Solar(lunarDate,lunarMonth,lunarYear,1,timeZone);
	// console.log(l_curent_date);
	// if ((l_curent_date[0] == 0 && l_curent_date[1] == 0 && l_curent_date[2] == 0) || !(l_curent_date[0] == f_curent_date[0] && l_curent_date[1] == f_curent_date[1] && l_curent_date[2] == f_curent_date[2])) {
	if ((l_curent_date[0] != 0 && l_curent_date[1] != 0 && l_curent_date[2] != 0) && !(l_curent_date[0] == f_curent_date[0] && l_curent_date[1] == f_curent_date[1] && l_curent_date[2] == f_curent_date[2])) {
		return true;
	}
	return false;
