...
public class CalendarProviderAccessor extends AbstractCalendarAccessor {
  ...
  protected EnumMap<KeyIndex, String> initContentProviderKeys() {
    ...
    keys.put(KeyIndex.GUESTS_CAN_MODIFY, Events.GUESTS_CAN_MODIFY);
    keys.put(KeyIndex.ORIGINAL_ID, Events.ORIGINAL_ID);
    keys.put(KeyIndex.AVAILABILITY, Events.AVAILABILITY);
    keys.put(KeyIndex.ORIGINAL_SYNC_ID, Events.ORIGINAL_SYNC_ID);
    keys.put(KeyIndex.DURATION, Events.DURATION);
    keys.put(KeyIndex.ACCESS_LEVEL, Events.ACCESS_LEVEL);
    return keys;
  }
