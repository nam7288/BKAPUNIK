...
- (void) deleteEventFromCalendar:(CDVInvokedUrlCommand*)command
    ...
    NSDictionary* options = [command.arguments objectAtIndex:0];
    NSString* list = [options objectForKey:@"notes"];
    NSArray * splitNote = [list componentsSeparatedByString:@"~"];
    NSString* notes      = splitNote[0];
    if([notes isEqualToString:@"undefined"])
    {
        notes = nil;
    }
    NSString* deleteFuture = splitNote[1];
    ...
    [self.commandDelegate runInBackground: ^{
        ...
        for (EKEvent * event in matchingEvents) {
            ...
            //[self.eventStore removeEvent:event span:EKSpanThisEvent error:&error]; // chi xoa event duoc chon
            if([deleteFuture isEqualToString:@"0"])
            {
...
- (NSMutableArray*) eventsToDataArray: (NSArray*)matchingEvents {
  ...
  for (EKEvent * event in matchingEvents) {
    NSMutableDictionary *entry = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
              ...
              {
                  NSTimeInterval offset = [[event.alarms objectAtIndex:i] relativeOffset];
                  
                  if(offset == -900)
                  {
//                      NSLog(@"alarm: 15m");
                      [entry setObject:@"15" forKey:@"reminder"];
                  }
                  
                  if(offset == -1800)
                  {
//                      NSLog(@"alarm: 15m");
                      [entry setObject:@"30" forKey:@"reminder"];
                  }
                  
                  if(offset == -3600)
                  {
//                      NSLog(@"alarm: 30m");
                      [entry setObject:@"60" forKey:@"reminder"];
                  }
                  
                  if(offset == -7200)
                  {
//                      NSLog(@"alarm: 60m");
                      [entry setObject:@"120" forKey:@"reminder"];
                  }
                  
                  if(offset == -86400)
                  {
//                      NSLog(@"alarm: 1d");
                      [entry setObject:@"1440" forKey:@"reminder"];
                  }
                  
                  if(offset == -172800)
                  {
//                      NSLog(@"alarm: 2d");
                      [entry setObject:@"2880" forKey:@"reminder"];
                  }
              }
...
- (void) findEventWithOptions:(CDVInvokedUrlCommand*)command {
  ...
  [self.commandDelegate runInBackground: ^{
    ...
    NSMutableArray * eventsDataArray = [self eventsToDataArray:matchingEvents];
      //EKAlarm *alarms = [[EKAlarm alloc] init];
      //NSDate *date = alarms.absoluteDate;
